<script lang="ts">
  ...
  let localRenderedVersion = $state('');
  let serverPreviewUrl = $state('');
  let serverPreviewVersion = $state('');
  ...
  const sceneInput = $derived.by<SceneInput | undefined>(() => {
    if (!hasSource) return undefined;
    return sourceMode === 'snippet'
      ? { kind: snippetMode, content: snippetContent, language, label: sourceLabel, siteUrl, options: { ...options } }
      : { kind: sourceMode as 'image' | 'website', imageDataUrl, sourceWidth, sourceHeight, label: sourceLabel, siteUrl, options: { ...options } };
  });
  const sceneVersion = $derived(`${sourceRevision}:${sourceMode}:${snippetMode}:${language}:${sourceLabel}:${snippetContent}:${JSON.stringify(options)}`);
  const previewDirty = $derived(supported === false && hasSource && serverPreviewVersion !== sceneVersion);
  const sceneLoading = $derived(serverLoading || (supported === true && hasSource && localRenderedVersion !== sceneVersion));

  async function updateServerPreview() {
    if (!hasSource) return;
    const requestedVersion = sceneVersion;
    serverLoading = true;
    try {
      const response = await fetch('/api/beautify', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify(requestBody('png'))
      });
      ...
      if (requestedVersion !== sceneVersion) {
        URL.revokeObjectURL(url);
        return;
      }
      const previousUrl = serverPreviewUrl;
      serverPreviewUrl = url;
      serverPreviewVersion = requestedVersion;
      if (previousUrl) setTimeout(() => URL.revokeObjectURL(previousUrl), 0);
    } catch (error) {
      renderFailure(error, 'Server preview unavailable.', 'server-preview');
    } finally {
      serverLoading = false;
    }
  }
  ...
  async function waitForLocalCanvas(): Promise<HTMLCanvasElement | null> {
    ...
    for (let attempt = 0; attempt < 90; attempt += 1) {
      if (previewCanvas && localRenderedVersion === sceneVersion) return previewCanvas;
      await new Promise((resolve) => setTimeout(resolve, 34));
    }
    return null;
  }

  function localRendered(canvas: HTMLCanvasElement, version: string | number) {
    previewCanvas = canvas;
    localRenderedVersion = String(version);
  }

  function localRenderFailed(error: unknown) {
    renderFailure(error, 'Preview unavailable.', 'local-preview');
  }
...
<main>
  ...
  <section class="workspace" class:is-unsupported={supported === false}>
    ...
    >
      ...
      {#if capturing}<div class="capture-progress" aria-live="polite"><span></span><strong>Capturing the full page…</strong><small>{websiteViewport === 'desktop' ? '1440 px desktop' : '390 px mobile'}</small></div>{/if}
      {#if supported === true && sceneInput}
        <div class="canvas-preview">
          <CanvasRenderer input={sceneInput} version={sceneVersion} fit bind:canvas={previewCanvas} onrender={localRendered} onerror={localRenderFailed} />
        </div>
        {#if sceneLoading}<div class="stage-loading overlay"><Sparkles size={24} /> Rendering…</div>{/if}
        {#if visualSource && options.fit === 'cover'}<span class="zoom-pill">{Math.round(options.zoom * 100)}%</span>{/if}
      {:else if supported === false && serverPreviewUrl}
        <img class="server-preview" src={serverPreviewUrl} alt="DevShot server-rendered preview">
        {#if previewDirty}<button class="stale-preview" onclick={updateServerPreview} disabled={serverLoading}>Update preview</button>{/if}
        {#if visualSource && options.fit === 'cover'}<span class="zoom-pill">{Math.round(options.zoom * 100)}%</span>{/if}
      {:else if sceneLoading}
        <div class="stage-loading"><Sparkles size={24} /> Building the frame…</div>
      {:else if hasSource}
        <div class="stage-unavailable"><Sparkles size={24} /><strong>Preview ready to render.</strong><span>Your source is loaded. Server mode updates only when requested.</span><Button size="sm" variant="accent" onclick={updateServerPreview}>Update preview</Button></div>
      {:else}
        <div
          class:dragging
      ...
      {/if}
      {#if visualSource && hasSource && options.fit === 'cover'}
        <div class="pan-surface" role="presentation" onpointerdown={beginPan} onpointermove={movePan} onpointerup={endPan} onpointercancel={endPan}></div>
      {/if}
    </div>
    ...
    <aside class="controls">
      ...
      {:else}
        ...
        {#if visualSource}<fieldset><legend>LOOK</legend><div class="look-grid">{#each ['clean', 'pop', 'noir', 'acid', 'dream'] as value}<button class:active={options.look === value} onclick={() => options.look = value as BeautifyOptions['look']}>{value}</button>{/each}</div></fieldset>{/if}

        <fieldset><legend>FX</legend><div class="look-grid fx-grid">{#each ['none', 'glow', 'orbit', 'grid'] as value}<button class:active={options.effect === value} onclick={() => options.effect = value as BeautifyOptions['effect']}>{value}</button>{/each}</div></fieldset>

        <div class="field-grid"><label><span>SHAPE</span><select bind:value={options.aspectRatio}><option>original</option><option>1:1</option><option>16:9</option><option>4:3</option><option>3:2</option><option>2:1</option><option>9:16</option><option>4:5</option></select></label>{#if visualSource}<label><span>VIEW</span><select bind:value={options.fit} onchange={() => { if (options.fit === 'contain') resetFocus(false); }}><option value="contain">Fit</option><option value="cover">Zoom</option></select></label>{:else}<label><span>FORMAT</span><select bind:value={options.format}><option value="png">PNG</option><option value="jpeg">JPG</option></select></label>{/if}</div>
        ...
        {#if previewDirty}<Button variant="outline" size="sm" onclick={updateServerPreview} loading={serverLoading}>Update server preview</Button>{/if}
        <p class="microcopy">Local canvas when supported · on-demand Chrome Beta otherwise.</p>
      {/if}
    </aside>
  </section>
